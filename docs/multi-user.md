...
用户名为 3 至 32 位英文字母、数字、点、下划线或连字符，首位须为字母或数字；密码为 1 至 128 个字符。`.env` 密码用单引号包围，避免 `$` 和 `#` 被解析，含单引号的密码可改用密码文件。Compose 自动创建或复用固定名称的 `any-listen-data` 卷，程序创建卷内目录。旧部署若使用项目名前缀的卷，先修改 Compose 的卷 `name` 指向原卷，避免误用空卷。
